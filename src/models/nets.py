...
class ResNetBiggerBin7(nn.Module):
    """1D CNN Architecture with residual connections, even bigger
    """
    ...
    def __init__(self, input_dim=2700, output_dim=1, dropout=0.1):
        ...
        # two large kernsl to begin with with no downsampling
        self.block0 = ConvResBlock(in_channels=1, out_channels=64, kernel_size=7, padding="same", batch_normalization=False, dropout=0)
        self.block0a = ConvResBlock(in_channels=64, out_channels=64, kernel_size=5, padding="same", dropout=self.dropout)
        # downsample from now
        self.block1 = ConvResBlock(in_channels=64, out_channels=64, kernel_size=3, pooling_size=2, dropout=self.dropout)
        self.block2 = ConvResBlock(in_channels=64, out_channels=128, kernel_size=3, pooling_size=2, dropout=self.dropout)
        self.block3 = ConvResBlock(in_channels=128, out_channels=128, kernel_size=3, pooling_size=2, dropout=self.dropout)
        self.block4 = ConvResBlock(in_channels=128, out_channels=256, kernel_size=3, pooling_size=2, dropout=self.dropout)
        self.block5 = ConvResBlock(in_channels=256, out_channels=256, kernel_size=3, pooling_size=2, dropout=self.dropout)
        self.block6 = ConvResBlock(in_channels=256, out_channels=512, kernel_size=3, pooling_size=2, dropout=self.dropout)
        self.block6a = ConvResBlock(in_channels=512, out_channels=512, kernel_size=3, pooling_size=2, dropout=self.dropout)
        self.block6b = ConvResBlock(in_channels=512, out_channels=256, kernel_size=3, pooling_size=2, dropout=self.dropout)
        # self.block6c = ConvResBlock(in_channels=128, out_channels=128, kernel_size=3, pooling_size=2, dropout=self.dropout)
        self.block7 = ConvResBlock(in_channels=256, out_channels=128, kernel_size=3, dropout=self.dropout)
